...
def dashboard(request):
    """
    Render dashboard with device and VM counts.
    """
    context = {
...
# -------------------------
# IP Prefix Checker View
# -------------------------
@method_decorator(csrf_exempt, name='dispatch')
class IPPrefixCheckerView(View):
    """
    View for checking IP prefixes and availability.
    """
    template_name = "nbtools/ip_prefix_checker.html"

    def get(self, request):
        """
        Display initial form with VRFs.
        """
    ...
    def post(self, request):
        """
        Handle prefix checking actions.
        """
        vrfs = VRF.objects.all()
        prefixes = []
        results = []
    ...
    def calculate_prefix_stats(self, prefix_obj, skip_azure=False):
        """
        Calculate statistics for a given prefix.
        """
...
# -------------------------
# Prefix Validator View
# -------------------------
@method_decorator(csrf_exempt, name='dispatch')
class PrefixValidatorView(View):
    """
    View for validating overlapping prefixes between VRFs.
    """
    template_name = "nbtools/prefix_validator.html"

    def get(self, request):
        vrfs = VRF.objects.all()
        return render(request, self.template_name, {"vrfs": vrfs, "results": []})
    ...
    def post(self, request):
        ...
        return render(request, self.template_name, {"vrfs": vrfs, "results": results})


# -------------------------
# Documentation Reviewer View
# -------------------------
class DocumentationReviewerView(View):
    """
    View for reviewing and updating documentation fields on Devices and VMs.
    """
    ...
    def _mark_reviewed(self, request):
        """
        Mark selected objects as reviewed.
        """
    ...
    def _check_fields(self):
        """
        Ensure custom fields exist and update missing ones.
        """
        for model in [Device, VirtualMachine]:
    ...
    def _set_custom_fields_in_batches(self, queryset):
        """
        Update custom fields in batches for performance.
        """
        BATCH_SIZE = 100
    ...
    def _render_page(self, request, search_query=""):
        """
        Render the documentation reviewer page with flagged objects.
        """
        flagged_objects = []
        valid_objects = 0
...
class VMToolView(View):
    """
    View for managing Virtual Machines and their IP assignments.
    """
    template_name = "nbtools/vm_tool.html"

    def get(self, request):
        """
        Display initial VM tool page.
        """
        return render(request, self.template_name, {
            "mode": "initial",
    ...
    def handle_existing_vm(self, request, error_message=""):
        """
        Handle operations on existing VMs.
        """
    ...
    def apply_changes(self, request):
        """
        Apply changes to VM and assign IP.
        """
        vm_id = request.POST.get("vm")
        interface_id = request.POST.get("interface")
        prefix_id = request.POST.get("prefix")
...
class SerialChecker(View):
    """
    View for checking and generating serial numbers for VMs.
    """
